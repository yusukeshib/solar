...
class Cylinder extends React.PureComponent {
  ...
  async update(props) {
    const { type, objRef, position, rotation, color, map, height, radius, segments, scene } = props || this.props
    if(this._node) {
      scene.remove(this._node)
    }
    this._geometry = new THREE.CylinderGeometry(radius, radius, height, segments)
    if(type === 'lambert') {
...
export default withSceneContext(Cylinder)
